...
def main(model_path):
    #to sample the model by farthest points sampling and save to a txt file
    # model_path: the 3d model path 
    
    model=dataset_utils.load_ply_model(os.path.join(model_path,'mesh.ply'))/1000
    
    num_sample=8 # can use the other number 

    index=fps_n(model,num_sample)

    index_np=index.numpy()

    print(index_np)

    fps_points=model[index_np]

    np.savetxt(os.path.join(model_path,'fps_n.txt'),fps_points)
